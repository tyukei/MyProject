...
import { AIWebsocket } from "./ai-websocket.js";
...
export class AIBot {
  ...
  /**
   * 指定された SceneId に接続している WebRtcPlayer のミュート設定を行います。
   * 
   * @param {String} sceneId シーンID
   * @param {boolean} muted ミュート設定
   */
  ...
  createWebRtcPlayer(sceneId, datachannel = false) {
    let webRtcPlayerObj = new WebRTCPlayer(sceneId, this.config, datachannel);
  ...
  processStats(data, webRtcPlayerObj) {
    ...
    if (view[0] === ToClientMessageType.QualityControlOwnership) {
      ...
      console.error(`unrecognized data received, packet ID ${view[0]}`, data);
    }
