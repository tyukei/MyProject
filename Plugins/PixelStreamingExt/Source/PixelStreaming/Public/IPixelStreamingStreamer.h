...
public:
	...
	// シグナリンサーバに接続します。
	// シグナリンサーバ接続完了イベントは、FStreamingStartedEvent を使用します。
	virtual void ConnectToSignallingServer() = 0;
	// シグナリンサーバから切断します。
	// シグナリンサーバ切断完了イベントは、FStreamingStoppedEvent を使用します。
	virtual void DisconnectToSignallingServer() = 0;
	// 映像配信を開始します。
	virtual void StartStreamingExt() = 0;
	// 映像配信を停止します。
	virtual void StopStreamingExt() = 0;

	/**
	 * @brief Send a message to the player.
	 * @param PlayerId Destination of the player
	 * @param Type The message type to be sent to the player
	 * @param Descriptor The contents of the message
	 */
	virtual void SendPlayerMessage(FPixelStreamingPlayerId PlayerId, uint8 Type, const FString& Descriptor) = 0;

	// WebRTC 接続完了イベントを定義します。
	DECLARE_EVENT_OneParam(IPixelStreamingStreamer, FStreamingCompletedEvent, IPixelStreamingStreamer*);
	// WebRTC 接続完了イベントのデリゲートを取得します。
	virtual FStreamingCompletedEvent& OnStreamingCompleted() = 0;
	// PluginExt: END
	////////////////////////////////////////////////////////////////////////////////////////////////
};
