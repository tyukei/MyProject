...
namespace UE::PixelStreaming
{
	...
	bool FStreamer::CreateSession(FPixelStreamingPlayerId PlayerId)
	{
		if (FPlayerContext* PlayerContext = Players.Find(PlayerId))
		{
			...
			else
			{
                ...
                ////////////////////////////////////////////////////////////////////////////////////////////////////////////
				// PluginExt: START
				// ICE の接続状態から WebRTC の接続状態を通知します。
				// kIceConnectionCompleted で通信経路が完了していますので、接続できたことを通知します。
				NewConnection->OnIceStateChanged.AddLambda([this, PlayerId](webrtc::PeerConnectionInterface::IceConnectionState State) {
					if (State == webrtc::PeerConnectionInterface::IceConnectionState::kIceConnectionCompleted)
					{
						OnStreamingCompleted().Broadcast(this);
					}
				});
				// PluginExt: END
				////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void FStreamer::OnPlayerConnected(FPixelStreamingPlayerId PlayerId, const FPixelStreamingPlayerConfig& PlayerConfig, bool bSendOffer)
	{
		////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// PluginExt: START
		// Player が接続されたときのイベントを発生させます。
		if (UPixelStreamingDelegates* Delegates = UPixelStreamingDelegates::GetPixelStreamingDelegates())
		{
			Delegates->OnPlayerConnectedNative.Broadcast(StreamerId, PlayerId);
		}
		// PluginExt: END
		////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void FStreamer::ConsumeStats(FPixelStreamingPlayerId PlayerId, FName StatName, float StatValue)
	{
		if (StatName == PixelStreamingStatNames::MeanQPPerSecond)
		{
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // PluginExt: START
            // PlayerId が SFU の場合、Players 登録の全ての PlayerId に対して送信するようにします。
            //
            if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
            {
                Players.Apply([&StatValue, this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
                    if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
                    {
	...
	void FStreamer::DeleteAllPlayerSessions()
	{
		////////////////////////////////////////////////////////////////////////////////////////
		// PluginExt: START
		SignallingServerConnection->SendDisconnectScene(StreamerId, TEXT("Stop scene capture."));

		// 登録されているプレイヤーのGoingAwayイベントを発行します。
		Players.Apply([this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
			if (UPixelStreamingDelegates* Delegates = UPixelStreamingDelegates::GetPixelStreamingDelegates())
			{
				if (PlayerContext.DataChannel)
				{
					Delegates->OnDataChannelClosedNative.Broadcast(StreamerId, DataPlayerId);
				}
				Delegates->OnPlayerGoingAwayNative.Broadcast(StreamerId, DataPlayerId);
			}
		});
		// PluginExt: END
		////////////////////////////////////////////////////////////////////////////////////////
	...
	void FStreamer::OnDataChannelMessage(FPixelStreamingPlayerId PlayerId, uint8 Type, const webrtc::DataBuffer& RawBuffer)
	{
		...
		else if (Type == FPixelStreamingInputProtocol::ToStreamerProtocol.Find("TestEcho")->GetID())
		{
			...
			if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
			{
				Players.Apply([&RawBuffer, this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
					if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
					{
	...
	void FStreamer::SendInitialSettings(FPixelStreamingPlayerId PlayerId) const
	{
        ...
        if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
        {
            const_cast<TThreadSafeMap<FPixelStreamingPlayerId,UE::PixelStreaming::FPlayerContext> &>(Players).Apply([&FullPayload, this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
                if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
                {
                    ...
                    {
                        UE_LOG(LogPixelStreaming, Log, TEXT("Failed to send initial Pixel Streaming settings to player %s."), *DataPlayerId);
                    }
                }
            });
        }
	...
	void FStreamer::SendProtocol(FPixelStreamingPlayerId PlayerId) const
	{
		...
		for (EPixelStreamingMessageDirection MessageDirection : PixelStreamingMessageDirections)
		{
			...
			if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
			{
				const_cast<TThreadSafeMap<FPixelStreamingPlayerId,UE::PixelStreaming::FPlayerContext> &>(Players).Apply([&body, this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
					if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
					{
						...
						{
							UE_LOG(LogPixelStreaming, Warning, TEXT("Failed to send Pixel Streaming protocol to player %s. This player will use the default protocol specified in the front end"), *DataPlayerId);
						}
					}
				});
			}
	...
	void FStreamer::SendPeerControllerMessages(FPixelStreamingPlayerId PlayerId) const
	{
		...
		if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
		{
			const_cast<TThreadSafeMap<FPixelStreamingPlayerId,UE::PixelStreaming::FPlayerContext> &>(Players).Apply([this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
				if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
				{
					const uint8 ControlsInput = (Settings::GetInputControllerMode() == Settings::EInputControllerMode::Host) ? (DataPlayerId == InputControllingId) : 1;
					const uint8 ControlsQuality = DataPlayerId == QualityControllingId ? 1 : 0;
	...
	void FStreamer::SendLatencyReport(FPixelStreamingPlayerId PlayerId) const
	{
		...
		AsyncTask(ENamedThreads::GameThread, [this, PlayerId, ReceiptTimeMs]() {
			...
			if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
			{
				const_cast<TThreadSafeMap<FPixelStreamingPlayerId,UE::PixelStreaming::FPlayerContext> &>(Players).Apply([&ReportToTransmitJSON, this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
					if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
					{
	...
	void FStreamer::SendCachedFreezeFrameTo(FPixelStreamingPlayerId PlayerId) const
	{
		if (CachedJpegBytes.Num() > 0)
		{
			...
			if (PlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive))
			{
				const TArray64<uint8>& CopiedCachedJpegBytes = CachedJpegBytes;
				const_cast<TThreadSafeMap<FPixelStreamingPlayerId, UE::PixelStreaming::FPlayerContext>&>(Players).Apply([&CopiedCachedJpegBytes, this](FPixelStreamingPlayerId DataPlayerId, FPlayerContext& PlayerContext) {
					if (!DataPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive) && PlayerContext.DataChannel)
					{
						PlayerContext.DataChannel->SendArbitraryData(FPixelStreamingInputProtocol::FromStreamerProtocol.Find("FreezeFrame")->GetID(), CopiedCachedJpegBytes);
					}
				});
			}
			else
	...
	// PlayerId 指定でのメッセージ送信
	void FStreamer::SendPlayerMessage(FPixelStreamingPlayerId InPlayerId, uint8 Type, const FString& Descriptor)
	{
		if (InPlayerId.Equals(TEXT("SFU"), ESearchCase::CaseSensitive)) // PlayerId が SFU の場合、接続中の全 PlayerId にメッセージ送信
		{
			this->SendPlayerMessage(Type, Descriptor);
		}
		else // PlayerId が SFU 以外
		{
			FPlayerContext* PlayerContext = Players.Find(InPlayerId);
			if (PlayerContext && PlayerContext->DataChannel)
			{
				PlayerContext->DataChannel->SendMessage(Type, Descriptor);
			}
		}
	...
	// SFU へのセッションを作成します。
	void FStreamer::CreateSFUSession()
	{
		// TODO: SFU の ID を定義をするべきか検討が必要です。
		FPixelStreamingPlayerId PlayerId = ToPlayerId(TEXT("SFU"));

		FPlayerContext& PlayerContext = Players.GetOrAdd(PlayerId);
		PlayerContext.Config.SupportsDataChannel = true;
		PlayerContext.Config.IsSFU = true;
	...
	// SFU のセッションを破棄します。
	void FStreamer::DestroySFUSession()
	{
		FPixelStreamingPlayerId PlayerId = ToPlayerId(TEXT("SFU"));
		DeletePlayerSession(PlayerId);
	...
	{
		// TODO: 2回呼び出された場合はどうなるのか？検討が必要です。
		if (CurrentSignallingServerURL.IsEmpty())
		{
			UE_LOG(LogPixelStreaming, Log, TEXT("Attempted to start streamer (%s) but no signalling server URL has been set. Use Streamer->SetSignallingServerURL(URL) or -PixelStreamingURL="), *StreamerId);
			return;
	...
	void FStreamer::DisconnectToSignallingServer()
	{
		if (UPixelStreamingDelegates* Delegates = UPixelStreamingDelegates::GetPixelStreamingDelegates())
		{
			Delegates->OnStatChangedNative.Remove(ConsumeStatsHandle);
	...
	void FStreamer::StartStreamingExt()
	{
		bStreamingStarted = true;

		// WebRTC 向けの映像配信を開始します。
		VideoSourceGroup->Start();

		// SFU サーバに接続されている場合には、
		// WebRTC のセッションを作成して配信を開始します。
		if (bSfuConnected)
		{
			CreateSFUSession();
		}
	...
	void FStreamer::StopStreamingExt()
	{
		VideoSourceGroup->Stop();
		TriggerMouseLeave(StreamerId);
	...
	void FStreamer::clearGamepadAnalog(FPixelStreamingPlayerId PlayerId, const uint8 AxisIndex)
	{
		const uint8 MsgType = FPixelStreamingInputProtocol::ToStreamerProtocol.Find("GamepadAnalog")->GetID();
		const uint8 ControllerId = PlayerIdToInt(PlayerId) & 0xFF;
		const char data[] = {
			MsgType,							   // MsgType
			ControllerId,						   // ControllerIndex
			AxisIndex,							   // axisIndex
			0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 // analogValue
		};
		const webrtc::DataBuffer Buffer(rtc::CopyOnWriteBuffer(data, sizeof(data)), true);
		OnDataChannelMessage(PlayerId, MsgType, Buffer);
	}

	void FStreamer::clearGamepadButton(FPixelStreamingPlayerId PlayerId, const uint8 ButtonIndex)
	{
		const uint8 MsgType = FPixelStreamingInputProtocol::ToStreamerProtocol.Find("GamepadButtonReleased")->GetID();
		const uint8 ControllerId = PlayerIdToInt(PlayerId) & 0xFF;
		const char data[] = {
			MsgType,	  // MsgType
			ControllerId, // ControllerIndex
	...
	// -----------------------------------------------------------------------------------------------
	// シグナリングサーバからのイベントを受信します。
	// -----------------------------------------------------------------------------------------------

	void FStreamer::OnPlayerDisconnected(FPixelStreamingPlayerId PlayerId)
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("  FStreamer::OnPlayerDisconnected: PlayerId=%s"), *PlayerId);

		// GamepadAnalog の値をクリアします。
		// 切断されたときに、Gamepad が押下されたままだと
		// PlayerController に入力が残ってしまうことがありますので、
		// ここでクリアしておきます。
		clearGamepadAnalog(PlayerId, 1);
		clearGamepadAnalog(PlayerId, 2);
		clearGamepadAnalog(PlayerId, 3);
		clearGamepadAnalog(PlayerId, 4);
		clearGamepadButton(PlayerId, 0);

		if (UPixelStreamingDelegates* Delegates = UPixelStreamingDelegates::GetPixelStreamingDelegates())
		{
			Delegates->OnPlayerDisconnectedNative.Broadcast(StreamerId, PlayerId);
		}

		DeletePlayerSession(PlayerId);
	}

	void FStreamer::OnPlayerGoingAway(FPixelStreamingPlayerId PlayerId)
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("  FStreamer::OnPlayerGoingAway: PlayerId=%s"), *PlayerId);
		...
		if (UPixelStreamingDelegates* Delegates = UPixelStreamingDelegates::GetPixelStreamingDelegates())
		{
			Delegates->OnPlayerGoingAwayNative.Broadcast(StreamerId, PlayerId);
		}

		DeletePlayerSession(PlayerId);
	}

	void FStreamer::OnSfuConnected()
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("  FStreamer::OnSfuConnected"));

		bSfuConnected = true;

		// StartStreaming が呼び出されている場合には、
		// WebRTC のセッションを作成して配信を開始します。
		if (bStreamingStarted)
		{
			CreateSFUSession();
	...
	void FStreamer::OnSfuDisconnected()
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("  FStreamer::OnSfuDisconnected"));

		bSfuConnected = false;

		// SFU サーバへの接続が切断されたのでセッションを破棄します。
		DestroySFUSession();
	}

	void FStreamer::OnStreamerDataChannelsFailed(FPixelStreamingPlayerId PlayerId, const int SendStreamId, const int RecvStreamId)
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("FStreamer::OnStreamerDataChannelsFailed"));

		SignallingServerConnection->SendStreamerDataChannelsFailed(PlayerId, SendStreamId, RecvStreamId);
	}

	void FStreamer::OnAIConnected(FPixelStreamingPlayerId PlayerId, const FPixelStreamingPlayerConfig& PlayerConfig, bool bSendOffer)
	{
		if (!bSendOffer)
		{
			// If we're not sending the offer, don't create the player session
	...
	void FStreamer::OnAIDisconnected(FPixelStreamingPlayerId PlayerId)
	{
		DeletePlayerSession(PlayerId);
	}
	// PluginExt: END
