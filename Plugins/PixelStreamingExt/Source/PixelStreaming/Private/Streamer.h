...
namespace UE::PixelStreaming
{
	...
	// PluginExt: START
	public:
		virtual FStreamingCompletedEvent& OnStreamingCompleted() override
		{
			return StreamingCompletedEvent;
		};

		// シグナリングサーバに接続を行います.
		virtual void ConnectToSignallingServer() override;
		virtual void DisconnectToSignallingServer() override;

		// 映像配信・停止を行います。
		virtual void StartStreamingExt() override;
		virtual void StopStreamingExt() override;

		// PlayerId指定でメッセージ送信します.
		virtual void SendPlayerMessage(FPixelStreamingPlayerId PlayerId, uint8 Type, const FString& Descriptor) override;

	private:
		// SFU との接続を行います。
		void CreateSFUSession();
		void DestroySFUSession();

		// シグナリングサーバからのイベントを受信します。
		void OnPlayerDisconnected(FPixelStreamingPlayerId PlayerId);
		void OnPlayerGoingAway(FPixelStreamingPlayerId PlayerId);
		void OnSfuConnected();
		void OnSfuDisconnected();

		void OnAIConnected(FPixelStreamingPlayerId PlayerId, const FPixelStreamingPlayerConfig& PlayerConfig, bool bSendOffer);
		void OnAIDisconnected(FPixelStreamingPlayerId PlayerId);

		// シグナリングサーバへDataChannelの生成失敗を通知します
		void OnStreamerDataChannelsFailed(FPixelStreamingPlayerId PlayerId, const int SendStreamId, const int RecvStreamId);

		// Gamepad の初期化を行います。
		void clearGamepadAnalog(FPixelStreamingPlayerId PlayerId, const uint8 AxisIndex);
		void clearGamepadButton(FPixelStreamingPlayerId PlayerId, const uint8 ButtonIndex);

	private:
		FStreamingCompletedEvent StreamingCompletedEvent;

		// SFU サーバに接続されているかを管理するフラグ.
		FThreadSafeBool bSfuConnected;
	// PluginExt: END
	//////////////////////////////////////////////////////////////////////////////////////////////////
	};
} // namespace UE::PixelStreaming
